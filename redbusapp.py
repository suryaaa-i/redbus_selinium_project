...
if web=="Home":
    ...
    slt.markdown('''SQLITE 3: With help of SQLITE3 to establish a connection to a database, enabling seamless integration of the transformed dataset
    ...
    slt.markdown("Selenium, Python, Pandas,SQLITE3, Streamlit.")
    slt.subheader(":blue[Developed-by:]  Imandi Sai Surya")

# States and Routes page setting
if web == "📍States and Routes":
    S = slt.selectbox("Lists of States", [ "Andhra Pradesh", "Bihar", "Chattisgarh", "Goa", 
                                          "Punjab", "Rajasthan", "Sikkim", "Telangana","Uttar pradesh", "West Bengal"])
    ...
    if S == "Bihar":
        B = slt.selectbox("List of routes",lists_Bi)
   ...
   # Chattisgarh bus fare filtering
    if S == "Chattisgarh":
    ...
    if S=="Rajastan":
        ...
        def type_and_fare_R(bus_type, fare_range):
            conn = sqlite3.connect("red_bus_details.dbb")
            my_cursor = conn.cursor()
            # Define fare range based on selection
            if fare_range == "50-1000":
                fare_min, fare_max = 50, 1000
    ...
    if S=="Sikkim":
        S=slt.selectbox("list of rotes",lists_S)
        ...
        df_result = type_and_fare_S(select_type, select_fare)
        slt.dataframe(df_result)
    ...
    if S=="Telangana":
        T=slt.selectbox("list of routes",lists_TS)
